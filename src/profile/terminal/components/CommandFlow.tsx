...
for (const path in rawFiles) {
  ...
  if (fileName) {
    VIRTUAL_FILES[fileName] = rawFiles[path];
  }
}
...
export default function CommandFlow(props: CommandFlowProps) {
  ...
  const executeCommand = (rawCmd: string) => {
    ...
    switch (mainCmd) {
      ...
      case "cat":
        ...
        } else if (VIRTUAL_FILES[targetFile]) {
          output = VIRTUAL_FILES[targetFile];
        } else {
  ...
  return (
    <div
      class="font-mono text-sm leading-relaxed text-gray-200 min-h-[160px]"
        ...
        {(item) => (
          <div class="mb-2">
            <div class="flex items-center gap-2">
              <span class="text-green-400">{promptText()}</span>
